 ...
 *
 * This method is for internal use only.  The function tokenizeString returns
...
/* tokenizeString
 * input: a zero terminated string
 * output: an array of dynamically strings which contains the ' ' separated
 * tokens of str, uses retTokenCount to return by reference the number of tokens
 * in the array.
 *
 * Note: This function mallocs space.  The caller is responsible for freeing
 * this malloc-ed array of strings.
 */
char **tokenizeString(char *str, int *retTokenCount) {
  ...
  while (index < strLength) {
    ...
    buffer[bufferIndex] =
        '\0'; /* Put null terminator at end of buffer string */
